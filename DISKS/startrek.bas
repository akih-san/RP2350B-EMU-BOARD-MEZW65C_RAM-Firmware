...
480 IF K3=0 THEN FOR A=1 TO 3:FOR B=1TO3:K(A,B)=0:NEXTB:NEXTA:GOTO 520
...
650 IF S<200*K3 THEN PRINT AR$;"Shield Energy too low":SL=1:A=6:GOTO750
660 A=2: GOTO 750
670 PRINT
680 PRINT D$(2):FOR A=1 TO 8:FOR B=1 TO 8:? A$(S(A,B));:NEXT B:?:NEXTA
...
730 IF(A>10)OR(A<0) THEN ?"WARNING - PROPER COMMANDS ONLY !! ":GOTO710
740 IF A=1 THEN 780
...
850 ? AR$"REJECTED ! Course must be in range of 0 to 359.99 degrees."
...
1120 ?:D(R1)=0:PRINT"Damage Control report: ";D$(R1);" repaired.":?
...
1180 ?AR$;"Captain, heed Starfleet regulations! ( STAY IN GALAXY ) !!"
1190 T=T+W1/24:GOTO 710
1220 FOR X=0 TO INT(W1)
1230 IFNOT(Q1=8-INT(FNB1(X)/8)ANDQ2=INT(FNA1(X)/8+1))THEN X=W1:GOTO1400
...
2285 GOTO 710
2290 IF (C$="D") AND K3 THEN ?"Starbase protects Enterprise":GOTO 710
2310 PRINT
2320 FOR I=1 TO 3
...
2850 ? AR$;"Safety check -- Do you wish to abort mission":INPUT AN$
...
2910 FORA=1TO8:FOR B=1TO8:IF G(A,B)=INT(G(A,B))THEN ?" ???";:GOTO 2940
...
3240 IF KA=1 THEN ? TK-T;" stardates left to save Starbase.":GOTO 3330
3250 IF (RND(0)>.01*R9) OR (B9=0) THEN RETURN
3260 KA=1:FOR A=1 TO 8:FOR B=1 TO 8
3270IFG(A,B)-100*INT(G(A,B)/100)>9THEN K1=A:K2=B:IFRND(0)>.5THENA=8:B=8
...
3380 IF(Q1=K1) AND (Q2=K2) THEN ?"You arrived in time! Starbase saved!"
3390 KA=0:GOTO 710
...
3810 ?"Command Central advises that your ranking-";X4;
3815 ?"-is in the expert"
3820 PRINT"category. Be advised that if you are not properly qualified"
3830 PRINT"your chances of avoiding failure are nil."
3840 PRINT
3850 ?"Do you wish reassignment to a less hazardous mission (Y or N)"
3860 INPUT AN$
...
4210 ?"Your ";D$(A);" will be inoperative for";I;" units of startime"
...
4310 ?" The tolerance factor for this mission is [+/-]";TF;" percent"
4320 PRINT
4330 RETURN
...
4380 GOTO 710
4390 ?:?"You are exactly on computed schedule; Proceed with mission"
4400 GOTO 710
4410 IF XM>TF THEN 4560:IF XM>(TF*1.5)THEN 4590
4420 ?:?"You are ahead of computed schedule, keep up the good work !!"
4430 GOTO 710
4440 ?:?"You are behind schedule and out of tolerance factor !!"
4450 ?"Command Control projects failure...you may surrender to enemy"
4460 EQ=1:SD=1:GOTO 3910
4470 YY=AC^X4
4490 PRINT AR$;"---------COMPUTER PROJECTION----------":PRINT
4500 ?"Projected likelihood of success for your mission =";YY;"percent"
...
4820 ?:?"Captain, despite your slight advantage, you are no match for"
...
4970 PRINT:GOTO 710
4980 ?:PRINT AR$;"You must be docked at Starbase for Damage Repair !!"
4990 PRINT:GOTO 710
5000 PRINT
...
5200 EH$=AR$
5210 ? EH$;"Your progress is";EG$;" within tolerance for this mission"
5220 PRINT
5230 IF EG$<>" not" THEN 2680
5240 IF XM=>0 THEN 5290
5250 PRINT:SD=1:KD=0
5260 ?"Captain, Command Central projects defeat by enemy. You are now"
5270 PRINT"authorized to surrender or to activate Self-Destruct system"
5280 GOTO 2680
5290 PRINT:SD=0:KD=1
5300 ?"Captain, Federation Command projects success for your mission."
5310 GOTO 4560
...
5510 ?" THESE ARE THE VOYAGES OF THE STARSHIP ENTERPISE. THE MISSION: "
5520 ?" TO SEEK NEW LIFE AND NEW CIVILIZATIONS; TO BOLDLY GO WHERE NO"
5530 ?"                      MAN HAS GONE BEFORE.":?:?:?:?
5540 ?:?:PRINT:PRINT:PRINT:PRINT:INPUT "Do you need instructions";Q9$
5550 IF UCASE$(LEFT$(Q9$,1))="N" THEN RETURN
5560 PRINT
5570 ?"        **********    WELCOME TO STARTREK    **********":PRINT
5580 ?"    You are in control of the Starship Entrprise. Your mission"
5590 PRINT "will be to rid the galaxy of Klingons."
5600 ?"    The known galaxy is mapped into 64 quadrants (8 X 8). Each"
5610 PRINT "quadrant is composed of 64 sectors (also 8 X 8)."
5620 ?"    A map of the galaxy, as represented by your on-board Com-"
5630 PRINT "puter, looks like this:":PRINT
5640 FOR Q8=1 TO 3:PRINT "      ??? ??? ??? ??? ??? ??? ??? ???":NEXTQ8
5650 PRINT "      ??? ??? ??? ??? 316 ??? ??? ???"
5660 FOR Q8=1 TO 4:PRINT "      ??? ??? ??? ??? ??? ??? ??? ???":NEXTQ8
5670 PRINT
5680 ?"The question marks represent unexplored quadrants. The 3-digit"
5690 ? "numbers encode information about quadrants. The first digit"
5700 ?"tells how many Klingons are in the quadrant; the second, how"
...
5820 PRINT "     . . . * . . . *"
5830 PRINT:PRINT
5840 ?"The Klingons are represented by 'K', stars by '*', Your ship by"
...
5870?"    In your pursuit of Klingons, you will have a large number of"
5880?"units of energy. These can by used to move the ship, to defend"
5890?"the ship, or to fire at Klingons. In executing any of the afore-"
5900?"mentioned functions, energy is expended. If you ever run out of"
5910 ?"energy, you will be immobile and defenseless.":PRINT:PRINT
5920 PRINT "    A SUMMARY OF TOP LEVEL COMMANDS FOLLOWS:"
5930 PRINT
5940 PRINT "---> LIST TOP LEVEL COMMANDS  (Activated by a '0')"
5950 ?"     This will list the commands you have available to you in"
5960 PRINT "     response to `COMMAND?'."
5970 PRINT
5980 PRINT "---> WARP  (Activated by entering a '1')"
5990 ?"     WARP moves the starship. Upon entering the command, you"
6000?"     will be asked for a direction of travel ( 0 to 360 degrees,"
...
6080 ?"     Displays information about your quadrant and it's eight"
...
6170 ? "     Computer-controlled high energy beams are fired at all"
6180 PRINT "     Klingons in the quadrant."
6190 PRINT
6200 PRINT "---> PHOTON TORPEDOES  ('5')"
6210 ?"     One of ten Torpedoes can be fired at a Klingon. They must"
...
6260 ? "     As the Klingons have weapons like yours, your ship has"
6270 ?"     shields to protect it. This command controls the energy"
6280 PRINT "     in the shields."
6290 PRINT
6300 PRINT "---> DAMAGE CONTROL  ('7')"
6310 ?"     Enables you to check the operational status of any of the"
6320 PRINT "     ship's functions. "
6330 PRINT
6340 PRINT "---> COMPUTER  ('8')"
6350 ?"     Accesses the on-board Computer. (See the list of computer"
6360 PRINT "     commands at the end of this list.)"
6370 PRINT
6380 PRINT "---> DAMAGE REPAIR  ('9')"
6390 ? "     This will allow you to repair damage to the Enterprise"
...
6450 ? "---> `0' Lists available commands from the Computer.":PRINT
6460 PRINT "---> GALACTIC MEMORY MAP   (`1')"
6470 ?"     This option lists all quadrants that the Enterprise has"
...
6540 ? "     This will assist in telling the Captain what direction"
...
6580 ?"     This is to be used on the first couple of turns to abort"
...
6620 ?"     If your Short-Range Scanners are out, this will give your"
6630 ?"     present position, but NOT what is in the quadrant.":PRINT
6640 PRINT "---> SELF-DESTRUCT   (`6')"
6650 ? "     If Security Control authorizes it, you may destroy the"
6660 PRINT "     Enterprise to avoid take-over by the Klingons.":PRINT
6670 PRINT "---> WARP-COMPASS   (`7')"
6680 ?"     With the Enterprise in the middle, this will help deter-"
6690 ?"     mine direction for movement and Photon Trajectory.":PRINT
6700 PRINT "---> MISSION PROGRESS REPORT   (`8')"
6710 ?"     This gives the Initial and Current Progress Factors as "
